...
/**
        ...
        const copyTexts = id => {
            document.querySelector(`#copy-text-btn${id}`).onclick = () => {
                const estilo = document.querySelector(`#text${id}`)
                copyText(estilo)
        ...
        copyTexts('')    
        const contexts = Array(105-1).fill('')
            .forEach((context, index) => copyTexts(index+2))
